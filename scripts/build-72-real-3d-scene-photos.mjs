...
function createPillOverlay(item) {
  const width = 1600;
  const height = 1000;
...
async function renderReal3dPhotoForBlog(item, idx) {
  ...
  const svgOverlay = Buffer.from(createPillOverlay(item));
...
async function run() {
  console.log("Generating REAL 3D PIXAR SCENE PHOTOS for ALL 72 Journal Essays...");

  for (let i = 0; i < blogList.length; i++) {
    await renderReal3dPhotoForBlog(blogList[i], i);
  }

  console.log("Successfully generated REAL 3D PIXAR SCENE PHOTOS for ALL 72 Journal Essays!");
}
